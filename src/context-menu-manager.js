...
module.exports = class ContextMenuManager {
  ...
  templateForEvent(event) {
    ...
    while (currentTarget) {
      currentTargetItems = []
      matchingItemSets = this.itemSets.filter((itemSet) => {currentTarget.webkitMatchesSelector(itemSet.selector)})
  
      for (const itemSet of matchingItemSets) {
  ...
  addAccelerators(template) {
    for (const id in template) {
      ...
      if (item.command) {
        ...
        const keystrokes = (keymaps) ? keymaps[0].keystrokes : null
        if (keystrokes) {
  ...
  pruneRedundantSeparators(menu) {
    var keepNextItemIfSeparator = false
    var index = 0
  ...
  cloneItemForEvent(item, event) {
    ...
    if (typeof item.shouldDisplay == 'function' &&  !item.shouldDisplay(event))
      return null
    if (item.created) item.created(event)
    if (Array.isArray(item.submenu))
